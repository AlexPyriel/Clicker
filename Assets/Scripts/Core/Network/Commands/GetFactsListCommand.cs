...
{
    public class GetFactsListCommand : IServerCommand
    {
        ...
        public async UniTask Execute()
        {
            try
            {
                ...
                if (result == UnityWebRequest.Result.Success)
                {
                    ...
                    Debug.Log($"[GetFactsListCommand] Executed. Facts Length:  {_factsModel.Breeds.Value.Count}");
                }
            }
            catch (Exception ex)
            {
    ...
    public class GetFactsListCommandFactory : PlaceholderFactory<GetFactsListCommand> {}
}